...
func (r *cpkExporter) Export() (*Result, error) {
	...
	if len(r.ram.Components) > 0 {
		...
		dirName := fmt.Sprintf("cpk.rbd.%v_v%v_%v", component.ServiceCname, r.ram.AppVersion, component.Arch)
...
func writeFilesDir(exportPath string, r *v1alpha1.Component, imageClient image.Client) error {
	...
	imageTarName := fmt.Sprintf("/cpk.rbd.%v_%v.tar", r.ServiceCname, r.DeployVersion)
...
func writeIconsDir(exportPath string, r *v1alpha1.Component, imageClient image.Client) error {
	...
	err = CopyFile("/run/rainbond.png", path.Join(exportPath, "icons", fmt.Sprintf("cpk.rbd.%v.png", r.ServiceCname)))
	if err != nil {
...
func packageJson(exportPath string, r *v1alpha1.Component, imageClient image.Client) error {
	pjStruct := cpk.PackageJSONCPK{
		...
		Glibc:          "",
		Id:             fmt.Sprintf("cpk.rbd.%v", r.ServiceCname),
		Name:           r.ServiceCname,
...
func writeScreeenshotsDir(exportPath string, r *v1alpha1.Component, imageClient image.Client) error {
	...
	err = CopyFile("/run/jt.png", path.Join(exportPath, "screenshots", fmt.Sprintf("cpk.rbd.%v_1.png", r.ServiceCname)))
	if err != nil {
...
func getFileImageJsonData(component *v1alpha1.Component) (string, error) {
	...
	for _, vo := range component.ServiceVolumeMapList {
		volumes = append(volumes, cpk.Volume{
			...
			HostPath:      path.Join(component.ServiceCname, vo.VolumeMountPath),
			Mode:          "RW",
		})
	}
	//处理port
	...
	cpkID := fmt.Sprintf("/cpk.rbd.%v-%v", component.ServiceCname, component.DeployVersion)
	appID := cpkID + cpkID

...
func (h *helmChartExporter) Export() (*Result, error) {
	...
	if err := PrepareExportDir(h.exportPath); err != nil {
...
func (h *helmChartExporter) initHelmChart() error {
	...
	if err := PrepareExportDir(helmChartPath); err != nil {
...
func (h *helmChartExporter) writeTemplateYaml(helmChartPath string) error {
	...
	if err := PrepareExportDir(helmChartTemplatePath); err != nil {
	...
	for _, k8sResource := range h.ram.K8sResources {
		...
		err = h.write(path.Join(helmChartPath, fmt.Sprintf("%v.yaml", unstructuredObject.GetKind())), unstructuredYaml)
		if err != nil {
			return err
		}
	}

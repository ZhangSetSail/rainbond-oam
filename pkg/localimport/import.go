...
func (r *ramImport) Import(filePath string, hubInfo v1alpha1.ImageInfo) (*v1alpha1.RainbondApplicationConfig, error) {
	...
	for _, com := range ram.Components {
		...
		options := docker2.ResolverOptions{
			Tracker: docker2.NewInMemoryTracker(),
			Hosts:   config.ConfigureHosts(r.ctr.CCtx, hostOpt),
		}
		r.logger.Infof("-----------------2-----------------")
		err = r.ctr.ContainerdClient.Push(r.ctr.CCtx, image.Name, image.Target, containerd.WithResolver(docker2.NewResolver(options)))
		if err != nil {
			return nil, err
		}
